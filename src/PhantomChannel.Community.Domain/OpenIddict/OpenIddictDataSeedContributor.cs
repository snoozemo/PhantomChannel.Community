    ...
    IStringLocalizer<OpenIddictResponse> l) : IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateApplicationBatchAsync(IConfigurationSection configurationSection, List<string> commonScopes)
    {
        ...
        foreach (var key in keys)
        {
            ...
            {
                var rootUrl = configurationSection[$"{key}:RootUrl"]?.TrimEnd('/') ?? null;
                var redirectUri = configurationSection[$"{key}:RedirectUris"] ?? null;
                var postLogoutRedirectUri = configurationSection[$"{key}:PostLogoutRedirectUri"] ?? null;
                await CreateApplicationAsync(
                    ...
                    scopes: commonScopes,
                    redirectUri: redirectUri == null ? null : $"{rootUrl}{redirectUri}",
                    postLogoutRedirectUri: postLogoutRedirectUri == null ? null : $"{rootUrl}{postLogoutRedirectUri}",
                    clientUri: rootUrl,
                    logoUri: "https://static.snoozemo.com/i/public/pch.fav.png"
            );
            }
        }
    }
